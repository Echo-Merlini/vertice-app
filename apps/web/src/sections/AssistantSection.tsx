...
export function AssistantSection() {
  ...
  // Seed the intro bot bubble once the (possibly API-hydrated) text is known.
  useEffect(() => {
  ...
  return (
    <section id="assistant" className="px-6 md:px-10 pt-24 md:pt-32 pb-4 max-w-3xl mx-auto scroll-mt-24">
      <FadeIn className="mb-6 text-center">
        <p className="font-mono text-[11px] uppercase tracking-[0.25em] text-brassLight/80">AI Assistant</p>
        <h2 className="mt-3 font-display font-medium tracking-tighter2 text-3xl sm:text-4xl md:text-5xl text-paper">
          {heading}
        </h2>
        <p className="mt-4 font-serif italic text-slate text-lg max-w-xl mx-auto">{sub}</p>
      </FadeIn>

      <FadeIn delay={0.1}>
        <div className="rounded-3xl border border-white/10 overflow-hidden" style={{ background: "#111319" }}>
          {/* messages */}
          <div ref={scrollRef} className="h-[300px] md:h-[340px] overflow-y-auto p-5 md:p-6 flex flex-col gap-4">
            {messages.map((m, i) =>
              m.role === "assistant" ? (
